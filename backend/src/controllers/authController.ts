  ...
  try {
    ...
    if (user) {
      ...
      const defaultCategories = [
        { name: 'Food', icon: 'utensils', color: '#EF4444', type: 'expense', userId: user._id, isDefault: true },
        { name: 'Travel', icon: 'plane', color: '#3B82F6', type: 'expense', userId: user._id, isDefault: true },
        { name: 'Shopping', icon: 'shopping-bag', color: '#8B5CF6', type: 'expense', userId: user._id, isDefault: true },
        { name: 'Bills', icon: 'file-text', color: '#F59E0B', type: 'expense', userId: user._id, isDefault: true },
        { name: 'Entertainment', icon: 'film', color: '#EC4899', type: 'expense', userId: user._id, isDefault: true },
        { name: 'Health', icon: 'heart', color: '#10B981', type: 'expense', userId: user._id, isDefault: true },
        { name: 'Salary', icon: 'dollar-sign', color: '#10B981', type: 'income', userId: user._id, isDefault: true },
        { name: 'Others', icon: 'more-horizontal', color: '#6B7280', type: 'expense', userId: user._id, isDefault: true },
      ];
  ...
  try {
    ...
    if (user) {
      ...
      if (req.body.email && req.body.email !== user.email) {
        ...
        const validOtp = await OTP.findOne({ email: req.body.email, otp: req.body.otp, type: 'email_update' });
        if (!validOtp) {
          return res.status(400).json({ message: 'Invalid or expired OTP for email update' });
        }
